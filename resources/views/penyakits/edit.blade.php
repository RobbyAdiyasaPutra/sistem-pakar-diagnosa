<x-app-layout> {{-- REMOVE @extends('layouts.app') AND @section('content') --}}
    ...
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Penyakit') }}: {{ $penyakit->nama_penyakit }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <form action="{{ route('penyakits.update', $penyakit->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <x-input id="kode_penyakit" class="block mt-1 w-full" type="text" name="kode_penyakit" :value="old('kode_penyakit', $penyakit->kode_penyakit)" required autofocus />
                        <x-input-error for="kode_penyakit" class="mt-2" />
                    </div>

                    <div class="mb-4">
                        <x-label for="nama_penyakit" value="Nama Penyakit" />
                        <x-input id="nama_penyakit" class="block mt-1 w-full" type="text" name="nama_penyakit" :value="old('nama_penyakit', $penyakit->nama_penyakit)" required />
                        <x-input-error for="nama_penyakit" class="mt-2" />
                    </div>

                    <div class="mb-4">
                        <x-label for="deskripsi" value="Deskripsi" />
                        <textarea id="deskripsi" name="deskripsi" rows="5" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('deskripsi', $penyakit->deskripsi) }}</textarea>
                        <x-input-error for="deskripsi" class="mt-2" />
                    </div>

                    <div class="mb-4">
                        <x-label for="solusi_umum" value="Solusi Umum" />
                        <textarea id="solusi_umum" name="solusi_umum" rows="3" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('solusi_umum', $penyakit->solusi_umum) }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="gejalas[]" id="gejalas" multiple class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            @foreach ($gejalas as $gejala)
                                <option value="{{ $gejala->id }}" {{ in_array($gejala->id, old('gejalas', $penyakit->gejalas->pluck('id')->toArray())) ? 'selected' : '' }}>
